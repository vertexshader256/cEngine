...
object UnaryExpression {
	...
	def execute(unary: IASTUnaryExpression)(implicit state: State): ValueType = {
		...
		unary.getOperator match
			...
			case `op_tilde` => processTilde(value)
			...
			case `op_sizeof` => processSizeof(value)
			case `op_amper` =>
				value match {
	...
	private def evaluateIncrDecr(unary: IASTUnaryExpression, value: ValueType, operator: Int)(implicit state: State): RValue = {
		...
		lValue.address.writeToMemory(newVal.value, lValue.theType)

		operator match
			case `op_postFixIncr` | `op_postFixDecr` => pre // push then set
			case `op_prefixIncr` | `op_prefixDecr` => newVal // set then push
	}
}
